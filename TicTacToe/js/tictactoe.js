...
function rollForTurn() {
    ...
    diceRoll(); //play dice sounds during the game roll fo rturn
...
function startGame() {
    ...
    if (activePlayer == "") { // if ti was a tie, then reroll
        activePlayer = rollForTurn();
    }
...
function recordMoves(square) {
    ...
    var info = boardState.split(','); // spearate the string by commas to create an array
...
function checkForWinCon() {
    ...
    // call this following array of functions to check ofr any of the possible win cons
...
function check4Tie() {
    ...
    if(boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!") {
        ...
        tieSound(); // play sound whne a tie has been detected
        writeMsg(txt1);
...
function glowBoard(pos) {
    ...
    for (var i=0; i<squares.length;i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(244,237,66)';}, 900)
...
function checkWinCon1(info,squareArray) {
    ...
    // do appear in the array, record their avatars
    // and compare all 3 for win cons
    for (var i in info) {
...
//checking wincon for squares 036
...
function square1Animate() {
    ...
    if (verdict == undefined) { // if verdict is empty than the square is unoccupied.
        ...
        if (paintAvatar == "O") { // change these all to ternary statements instead
            ...
            checkForWinCon(); // call function to check if current move complete a winning condition.
